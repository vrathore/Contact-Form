<?php 
	...
	/* If condition validating the captcha value entered, if valid, it'll insert a record in the CF table and will send the mail to admin*/
	if( $_SESSION['captcha'] == mysql_real_escape_string(trim($CF_captcha))) {
		$sql = "insert into $CF_table"
		. " set `CF_name`='" . mysql_real_escape_string(trim($CF_name))
		. "', `CF_email`='" . mysql_real_escape_string(trim($CF_email))
		. "', `CF_subject`='" . mysql_real_escape_string(trim($CF_subject))
		. "', `CF_message`='" . mysql_real_escape_string(trim($CF_message))
